<script lang="ts">
  import Image from "$lib/assets/images/langing/3.jpg";
  ...
  import { debounce, submitForm } from "@/lib/functions";

  export let data;

  let aparts = data.aparts;

  let form: HTMLFormElement;

  function setAparts(data: any) {
    aparts = data;
  }
  ...
  let person = 0; // Example rate (can be any value between 0 and 5)
  let maxPerson = 5; // Total number of stars
</script>
...
>
  <h2 class="relative z-10 text-2xl md:text-5xl font-bold">
    Apartman Fırsatları Kaçırmayın <br /> Son Gün 3 Ocak
  </h2>
...
<div class="grid md:grid-cols-4 gap-5 px-5 md:px-10 lg:px-16 my-5">
  <div class="md:col-span-1">
    ...
    <form
      ...
      use:enhance={() =>
        async ({ result }) => {
          if (result.type === "success") setAparts(result.data);
        }}
      bind:this={form}
      method="post"
    ...
    >
      ...
      <div>
        <h4 class="font-medium text-lg">Kaç kişisiniz</h4>
        <div class="flex justify-evenly mt-2 cursor-pointer">
          {#each Array(maxPerson).fill(0) as _, index}
            <button
              type="button"
              onclick={() => (person = index + 1)}
              aria-label={`Person ${index + 1} star${index + 1 > 1 ? "s" : ""}`}
            >
              <span
                class={`flex star font-medium w-10 h-10 items-center justify-center rounded-xl ${
                  index < person
  ...
  <div class="md:col-span-3">
    <h3 class="text-3xl font-bold text-slate-800 my-5">Apartmanlar</h3>
    <ul id="list" class="flex flex-col gap-5">
      {#each aparts as { id, name, description, location, price, capacity, rating, discounted_price }}
        <li
        ...
        >
          ...
          <div class="flex flex-col flex-1 gap-3">
            ...
            <span class="p-2 flex gap-2 bg-blue-100 w-fit rounded-lg">
              ...
              <p class="font-semibold">Kapasite: {capacity}</p>
            </span>
            <div class="flex-1 flex items-end justify-end">
              <Button
                onclick={() => goto(`/apartmanlar/${id}`)}
                variant="primary"
                type="button"
