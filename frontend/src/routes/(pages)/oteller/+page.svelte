<script lang="ts">
  ...
  import { debounce, submitForm, truncate } from "$lib/functions";
...
>
  <h2 class="relative z-10 text-2xl md:text-5xl font-bold">
    Oteller Fırsatları Kaçırmayın <br /> Son Gün 3 Ocak
  </h2>
  ...
  >
    ...
    <div class="relative flex-1 w-full">
      <Input
        ...
        name="name"
      />
      <Lucide icon="Hotel" className="absolute top-2 left-3 text-gray-500" />
    </div>
    <Button
      type="submit"
      variant="primary"
...
<div class="grid md:grid-cols-4 gap-5 px-5 md:px-10 lg:px-16 my-5">
  <div class="md:col-span-1">
    ...
    <form
      ...
      use:enhance={() =>
        async ({ result }) => {
          if (result.type === "success") setHotels(result.data);
        }}
      bind:this={form}
      method="post"
  ...
  <div class="md:col-span-3">
    <h3 class="text-3xl font-bold text-slate-800 my-5">Oteller</h3>
    <ul id="list" class="flex flex-col gap-5">
      {#each hotels as { id, name, description, location, rating }}
        <li
        ...
        >
          ...
          <div class="flex flex-col flex-1 gap-3">
            ...
            <a
              href={`/oteller/${id}`}
              class="flex-1 flex items-end justify-end"
            >
